...
class ProcessFrontFinalPriceObserver implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        //if (!$this->rulePricesStorage->hasRulePrice($key)) {
            $rulePrice = '30';//$this->resourceRuleFactory->create()->getRulePrice($date, $wId, $gId, $pId);
...
/*        if($product->getSku() == '1000'){
            $product->setFinalPrice(30);
            //$product->setPrice($product->getSku());

        }else{
            $product->setFinalPrice($product->getSku());
            //$product->setPrice($product->getSku());
        }*/
/*        $pId = $product->getId();
        $storeId = $product->getStoreId();
