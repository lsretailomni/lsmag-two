...
class ClientGenerate extends Command
{
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->odataCodeGeneration($entityDir, $operationDir);
    ...
    public function odataCodeGeneration($entityDir, $operationDir)
    {
        $odataMetaData          = $this->getOmniDataHelper()->fetchOdataV4();
        $odataBaseClassContent  = <<<PHP
<?php
...
PHP;
        $odataBaseClassFileName = AbstractGenerator::path($entityDir, "BaseODataRequest.php");
        // @codingStandardsIgnoreLine
        file_put_contents($odataBaseClassFileName, $odataBaseClassContent);

        $actions = $odataMetaData->query('//edm:Action[starts-with(@Name, "ODataRequest_")]');
        ...
        foreach ($actions as $action) {
            ...
            foreach ($params as $param) {
                if ($param->getAttribute('Name') === 'fullRepl') {
                    $hasFullRepl = true;
                    break;
                }
            }
...
class $name extends BaseODataRequest
{
...
PHP;
            $className = AbstractGenerator::path($entityDir, "{$name}.php");
            // @codingStandardsIgnoreLine
