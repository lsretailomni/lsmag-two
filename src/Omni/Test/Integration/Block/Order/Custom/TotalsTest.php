...
namespace Ls\Omni\Test\Integration\Block\Order\Custom;
...
class TotalsTest extends AbstractIntegrationTest
{
    ...
    /**
     * Build a Totals block with mocked parent; return totals added via addTotalBefore.
     *
     * @param array $posEntries  raw POS data entries (will be json_encoded)
     * @param float $pointsSpent ls_points_spent value
     */
    private function runInitTotals(array $posEntries, float $pointsSpent = 0.0): array
    {
        ...
        $parentMock = $this->getMockBuilder(AbstractBlock::class)
            ...
            ->addMethods(['getOrder', 'addTotalBefore'])
            ->getMock();
    ...
    public function testInitTotalsWithSingleGiftCardEntry(): void
    {
        ...
        $this->assertEquals(-40.0, $gcEntry->getValue());
    }
    ...
    {
        $totals = $this->runInitTotals([
            ['entry_type' => 'GIFTCARDNO', 'entry_no' => 'GC001', 'amount' => 25.0, 'tender_type' => ''],
            ['entry_type' => 'GIFTCARDNO', 'entry_no' => 'GC002', 'amount' => 25.0, 'tender_type' => ''],
        ]);
    ...
    {
        $totals = $this->runInitTotals([
            ['entry_type' => 'STOREVOUCHER', 'entry_no' => 'SV001', 'amount' => 12.5, 'tender_type' => ''],
        ]);
        ...
        $this->assertEquals(-12.5, $vEntry->getValue());
    }
