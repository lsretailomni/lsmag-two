...
class ReplicationHelper extends AbstractHelper
{
     ...
     * @return SearchCriteria
     */
    public function buildCriteriaForCategoryImages(
        array $filters,
        $parameter1,
        $parameter2,
        $pagesize = 100,
        ...
        $tableNameValue1 = $this->filterBuilder->setField($parameter1['field'])
            ->setValue($parameter1['value'])
            ->setConditionType($parameter1['condition_type'])
            ->create();
        $tableNameValue2 = $this->filterBuilder->setField($parameter2['field'])
            ->setValue($parameter2['value'])
            ->setConditionType($parameter2['condition_type'])
            ->create();
        // building OR condition between the above  criteria
        $filterCondition = $this->filterGroupBuilder
            ->addFilter($tableNameValue1)
            ->addFilter($tableNameValue2)
            ->create();
        $criteria        = $this->searchCriteriaBuilder->setFilterGroups([$filterCondition, $filterOr]);
